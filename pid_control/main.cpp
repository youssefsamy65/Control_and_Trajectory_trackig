...
//steer
 double kpSteer{0.5};
 double kiSteer{0.0007};
 double kdSteer{0.065};
 double maxSteer{1.2};
 double minSteer{-1.2};

//throotle
 double kpThrottle{0.2};
 double kiThrottle{0.0007};
 double kdThrottle{0.0006};
...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  pid_steer.Init(kpSteer, kiSteer, kdSteer,
                 maxSteer, minSteer);
  ...
  **/
  ...
  pid_throttle.Init(kpThrottle, kiThrottle, kdThrottle,
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          int min_dist_idx{0};
          for (int i=0; i < x_points.size(); ++i) {
            //calculate the current position distance from the planned 2arrays
